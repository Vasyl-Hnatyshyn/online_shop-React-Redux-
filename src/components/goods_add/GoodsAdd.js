...
const GoodsAdd = ({ addNewGoods, goodsData }) => {
  ...
  return (
    <div className="goods-add-wrapper">
      ...
      <button onClick={addNewGoodsToStore} className="add-goods-btn">
        {' '}
        Add Product
      </button>
      ...
      {error ? (
        <span className="error">Please fill in all required fields</span>
      ) : (
        ''
      )}
      {success ? (
        <span className="success">Product successfully added</span>
      ) : (
        ''
      )}
    </div>
